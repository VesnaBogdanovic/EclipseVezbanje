...
public class DynamicJson {
	...
	public void deleteBook(String isbn, String aisle) {
		RestAssured.baseURI = "http://216.10.245.166";
		String response = given().log().all().body(Payload.DeleteBook(isbn, aisle)).when()
				.post("/Library/DeleteBook.php").then().extract().response().asString();
		System.out.println("Response starts " + response + " ends");
	}

	@DataProvider(name = "BooksData")
	public Object[][] getData() {

		return new Object[][] { { "ojobm", "248" }, { "khfu", "548" }, { "dsogi", "448" } };
	}
}
